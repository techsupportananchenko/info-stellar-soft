...
//Simple fixed header
export function headerScrollFixed() {
  // Sticky header
...
export function digitsCounter() {
  ...
  if (document.querySelectorAll("[data-digits-counter]").length) {
    document.querySelectorAll("[data-digits-counter]").forEach(element => {
      // Значення з атрибута,тобто число
      element.dataset.digitsCounter = element.getAttribute('data-count-int');
      element.innerHTML = `0`;
    });
  }
